...
const Navbar = () => {
  ...
  useEffect(() => {
    const getUser = async () => {
      const res = await axios.get(
        `http://localhost:5000/api/auth/find/${user?.currentUser?._id}`
      );
  ...
  const fetchHotels = async (bookings) => {
    try {
      const hotelPromises = bookings.map(async (item) => {
        const res = await axios.get(
          `http://localhost:5000/api/hotel/find/${item?.hotelId?.id}`
        );
        return res.data;
      });
  ...
  const handleDelete = (hotelId)=>{
    const bookingDel = async()=>{
      await axios.put(`http://localhost:5000/api/auth/delete/${hotelId}/${user?.currentUser?._id}`);
      
    }
    const clearDates = bookings.map(async (item) => {
      await axios.put(
        `http://localhost:5000/api/room/booked/${item?.selectedRooms[0]}`
      );
